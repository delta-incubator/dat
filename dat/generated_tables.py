...
def reference_table(name: str, description: str):
    case = TestCaseInfo(name=name, description=description)
...
@reference_table(
    name="basic_append", description="A basic table with two append writes."
)
...
def create_basic_partitioned(case: TestCaseInfo, spark: SparkSession):
    ...
    df.repartition(1).write.partitionBy("letter").format("delta").save(case.delta_root)
    save_expected(case)

    data2 = [("a", 4, 4.4), ("e", 5, 5.5), ("f", 6, 6.6)]
    df = spark.createDataFrame(data2, schema=columns)

    df.repartition(1).write.partitionBy("letter").format("delta").mode("append").save(
        case.delta_root
    )
    save_expected(case)
...
@reference_table(
    ...
    description=(
        "A table with multiple partitioning columns. Partition "
...
def create_multi_partitioned(case: TestCaseInfo, spark: SparkSession):
    ...
    df.repartition(1).write.format("delta").partitionBy(*partition_columns).save(
        case.delta_root
    )
    save_expected(case)

    # Introduce null values in partition columns
    data2 = [("a", None, b"x", 4), (None, None, None, 5)]
...
@reference_table(
    ...
    description=(
        "Multiple levels of partitioning, with boolean, timestamp, and "
...
def with_schema_change(case: TestCaseInfo, spark: SparkSession):
    ...
    df.repartition(1).write.mode("overwrite").option("overwriteSchema", True).format(
        "delta"
...
def create_nested_types(case: TestCaseInfo, spark: SparkSession):
    schema = types.StructType(
        [
            types.StructField("pk", types.IntegerType()),
            types.StructField(
                "struct",
                types.StructType(
                    [
            ...
            types.StructField(
                "map", types.MapType(types.StringType(), types.IntegerType())
            ),
        ]
    )
...
def get_sample_data(
    spark: SparkSession, seed: int = 42, nrows: int = 5
...
def create_with_checkpoint(case: TestCaseInfo, spark: SparkSession):
    ...
    assert any(
        path.suffixes == [".checkpoint", ".parquet"]
...
def create_no_replay(case: TestCaseInfo, spark: SparkSession):
    ...
    assert any(path.suffixes == [".checkpoint", ".parquet"] for path in files_in_log)
...
@reference_table(name="check_constraints", description="Table with a check constraint")
def check_constraint_table(case: TestCaseInfo, spark: SparkSession):
    ...
    df.repartition(1).write.format("delta").mode("append").save(case.delta_root)
    spark.sql(
        f"ALTER TABLE delta.`{delta_path}` ADD CONSTRAINT const1 CHECK (int > 0);"
    )
...
def create_change_data_feed(case: TestCaseInfo, spark: SparkSession):
    ...
    delta_table.update(condition=col("letter") == "c", set={"letter": lit("a")})
    delta_table.delete(col("letter") == "a")
...
def create_generated_columns(case: TestCaseInfo, spark: SparkSession):
    ...
    (
        ...
        .addColumns(df.schema)
        .addColumn(
            "creation", types.DateType(), generatedAlwaysAs="CAST(now() AS DATE)"
        )
        .execute()
    )
