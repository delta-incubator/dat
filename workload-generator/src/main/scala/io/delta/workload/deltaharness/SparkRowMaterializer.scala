...
trait SparkRowMaterializer extends DeltaHarness {
  override def writeRows(
    ...
    val sparkRows = rows.map(r => coerce(Some(r), schema).asInstanceOf[Row])
  ...
  /**
   * Coerce a row's value to the column's Spark type. `None`/`null` -> SQL NULL, and struct/array/map
   * values recurse.
   */
      ...
      case Some(v) => dataType match {
        ...
        case st: StructType => v match {
          case m: Map[_, _] =>
            ...
            val unknown = fields.keySet -- st.fieldNames.toSet
            require(unknown.isEmpty,
              s"coerce: struct value has unknown field(s) ${unknown.mkString(", ")}; " +
                s"schema fields are ${st.fieldNames.mkString(", ")}")
