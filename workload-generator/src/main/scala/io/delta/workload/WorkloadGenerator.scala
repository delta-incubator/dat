...
import io.delta.workload.capture.{CdfCapture, CheckpointCapture, CrcCapture, ReadCapture, SnapshotCapture, TableInfoCapture}
...
object WorkloadGenerator {
  ...
  private[workload] def generateTable(
    ...
    // The cdf/checkpoint/crc specs need no name tracking (no dedup/latest handling), only a count
    ...
    for (cs <- ts.cdfSpecs) {
      val specPath = CdfCapture.capture(spark, dirName, destTablePath, testOutputDir, specsDir,
        ...
        predicate = cs.predicate, columns = cs.columns, expectError = cs.expectError)
      extraSpecCount += 1
    ...
    val total = snapshotNames.size + readNames.size + extraSpecCount
