...
class WriteBoundarySuite extends WorkloadTestSuite("write_boundary") {
  ...
  test("float_special_values") {
    val w = createTableOp("tbl",
      schema = new StructType().add("id", IntegerType).add("f", FloatType))
    insertOp(w, Seq(
      Map("id" -> 1, "f" -> Float.NaN),
      Map("id" -> 2, "f" -> Float.PositiveInfinity),
      Map("id" -> 3, "f" -> Float.NegativeInfinity),
      Map("id" -> 4, "f" -> 0.0f),
      Map("id" -> 5, "f" -> -0.0f)))
    val t = endWrite(w)
    readSpec(t)
  ...
  test("extreme_values_with_checkpoint") {
    // Extreme scalar values across two commits, with a forced checkpoint at the final version.
    val w = createTableOp("tbl",
      schema = new StructType().add("id", IntegerType).add("c_int", IntegerType)
        .add("c_long", LongType).add("ts", TimestampType).add("d", DecimalType(18, 0)))
    insertOp(w, Seq(Map("id" -> 1, "c_int" -> Int.MinValue, "c_long" -> Long.MinValue,
      "ts" -> "9999-12-31 23:59:59.999999", "d" -> "-999999999999999999")))
    insertOp(w, Seq(Map("id" -> 2, "c_int" -> Int.MaxValue, "c_long" -> Long.MaxValue,
      "ts" -> "9999-12-31 23:59:59.999999", "d" -> "999999999999999999")))
    val t = endWrite(w)
    readSpec(t)
