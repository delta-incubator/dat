...
import io.delta.workload.model.{AddDomainMetadata, AddFileInput, AppTxn}
...
class WriteSequencesSuite extends WorkloadTestSuite("write_sequences") {
  ...
  test("insert_update_delete_chain") {
    val w = createTableOp("tbl", schema = new StructType().add("id", IntegerType).add("status", StringType).add("n", IntegerType),
    ...
    readSpec(t, version = 2, name = Some("read_after_update"))
    readSpec(t, version = 3, name = Some("read_after_delete"))
    readSpec(t, predicate = "status = 'active'", name = Some("read_active"))
    readSpec(t, predicate = "n = 99", name = Some("read_n99"))
    snapshotSpec(t)
  }

  test("property_churn") {
    val w = createTableOp("tbl", schema = new StructType().add("id", IntegerType).add("v", IntegerType))
  ...
  test("schema_evolution_chain") {
    ...
    val w = createTableOp("tbl", schema = new StructType().add("id", IntegerType).add("a", StringType),
      properties = Map("delta.columnMapping.mode" -> "name"))
    insertOp(w, Seq(Map("id" -> 1, "a" -> "x")))
    addColumnsOp(w, new StructType().add("b", IntegerType))
    ...
    val t = endWrite(w)
    readSpec(t)
    readSpec(t, version = 1, name = Some("read_initial"))
    readSpec(t, version = 3, name = Some("read_after_add"))
    readSpec(t, predicate = "label = 'x'", name = Some("read_renamed"))
    snapshotSpec(t)
  }
  ...
  test("low_and_high_level_mix") {
    ...
    val t = endWrite(w)
    readSpec(t)
    readSpec(t, version = 2, name = Some("read_after_lowlevel_add"))
    snapshotSpec(t)
  }
