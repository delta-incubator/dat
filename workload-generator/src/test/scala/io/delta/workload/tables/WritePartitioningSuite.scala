...
class WritePartitioningSuite extends WorkloadTestSuite("write_partitioning") {
  ...
  test("empty_string_partition_value") {
    // A string partition column with an empty-string value alongside a normal value and NULL; the
    // empty string must round-trip through the partition-directory / partitionValues encoding.
    val w = createTableOp("tbl",
      schema = new StructType().add("id", IntegerType).add("p", StringType).add("v", IntegerType),
      partitionColumns = Seq("p"))
    insertOp(w, Seq(
      Map("id" -> 1, "p" -> "", "v" -> 1),
      Map("id" -> 2, "p" -> "normal", "v" -> 2),
      Map("id" -> 3, "p" -> null, "v" -> 3)))
    val t = endWrite(w)
    readSpec(t)
    readSpec(t, predicate = "p = ''", name = Some("read_empty_partition"))
    snapshotSpec(t)
  }

  test("decimal_partition_column") {
    // Partition by DECIMAL(10,2) with negative, zero, and positive values.
    val w = createTableOp("tbl",
      schema = new StructType().add("id", IntegerType).add("p", DecimalType(10, 2)).add("v", IntegerType),
      partitionColumns = Seq("p"))
    insertOp(w, Seq(
      Map("id" -> 1, "p" -> "-12.34", "v" -> 1),
      Map("id" -> 2, "p" -> "0.00", "v" -> 2),
      Map("id" -> 3, "p" -> "99.99", "v" -> 3),
      Map("id" -> 4, "p" -> "-12.34", "v" -> 4)))
    val t = endWrite(w)
    readSpec(t)
    readSpec(t, predicate = "p < 0", name = Some("read_negative_partition"))
    snapshotSpec(t)
  }
}
