 ...
 * narrowed indexed-column count, and timestamp/decimal ranges.
 */
class WriteStatsSuite extends WorkloadTestSuite("write_stats") {
  ...
  test("timestamp_ntz_and_boundary_subms_stats") {
    // Sub-millisecond fractions on both TIMESTAMP_NTZ and TIMESTAMP columns exercise micro-precision
    // min/max stats generation.
    val w = createTableOp("tbl",
      schema = new StructType().add("id", IntegerType)
        .add("ntz", TimestampNTZType).add("ts", TimestampType))
    insertOp(w, Seq(
      Map("id" -> 1, "ntz" -> "2024-01-01 00:00:00.000500", "ts" -> "2024-01-01 00:00:00.000500"),
      Map("id" -> 2, "ntz" -> "2024-01-01 00:00:00.000001", "ts" -> "2024-01-01 00:00:00.999999"),
      Map("id" -> 3, "ntz" -> "2024-01-01 00:00:00.999999", "ts" -> "2024-01-01 00:00:00.000500")))
    val t = endWrite(w)
    readSpec(t)
    snapshotSpec(t)
  }
